...
public class StickersQuery {
    ...
    public static void loadRecents(final int type, final boolean gif, boolean cache, boolean force) {
        ...
        if (cache) {
            ...
            SharedPreferences preferences = ApplicationLoader.applicationContext.getSharedPreferences("emoji_"+ApplicationLoader.currentUser, Activity.MODE_PRIVATE);
            if (!force) {
                long lastLoadTime;
                if (gif) {
    ...
    private static void processLoadedRecentDocuments(final int type, final ArrayList<TLRPC.Document> documents, final boolean gif, final int date) {
        ...
        if (date == 0) {
            AndroidUtilities.runOnUIThread(new Runnable() {
                @Override
                public void run() {
                    SharedPreferences.Editor editor = ApplicationLoader.applicationContext.getSharedPreferences("emoji_"+ApplicationLoader.currentUser, Activity.MODE_PRIVATE).edit();
                    if (gif) {
                        loadingRecentGifs = false;
                        recentGifsLoaded = true;
