...
public class ColorPicker extends FrameLayout {
    ...
    public ColorPicker(Context context) {
        ...
        location = context.getSharedPreferences("paint_"+ ApplicationLoader.currentUser, Activity.MODE_PRIVATE).getFloat("last_color_location", 1.0f);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (action == MotionEvent.ACTION_CANCEL || action == MotionEvent.ACTION_UP || action == MotionEvent.ACTION_POINTER_UP) {
            if (interacting && delegate != null) {
                ...
                getContext().getSharedPreferences("paint_"+ApplicationLoader.currentUser, Activity.MODE_PRIVATE).edit().putFloat("last_color_location", location).commit();
            }
            interacting = false;
